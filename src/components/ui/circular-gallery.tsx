...
interface MediaProps {
  ...
  viewport: Viewport; bend: number; textColor: string; borderRadius?: number;
  font?: string; onSelect?: (image: string) => void;
}
...
class Media {
  ...
  constructor({ geometry, gl, image, index, length, renderer, scene, screen, text, viewport,
    bend, textColor, borderRadius = 0, font, onSelect }: MediaProps) {
    ...
    this.font = font; this.onSelect = onSelect;
...
class App {
  ...
  screen!: ScreenSize; viewport!: Viewport; raf: number = 0;
  ...
  private _downInsideGallery: boolean = false;
  private _clickHandler!: (e: MouseEvent) => void;

  constructor(container: HTMLElement, {
    items, bend = 1, textColor = '#ffffff', borderRadius = 0,
    font = 'bold 30px Figtree', scrollSpeed = 2, scrollEase = 0.05, onSelect,
  }: AppConfig) {
  ...
  createMedias(items: { image: string; text: string }[] | undefined, bend: number,
  ...
  onTouchDown(e: MouseEvent | TouchEvent) {
    ...
    this._mouseDownX       = this.start;
    this._mouseDownTime    = Date.now();
    this._downInsideGallery = true;
  }
  ...
  addEventListeners() {
    ...
    // Click = mousedown + mouseup with little movement and quick time.
    // _downInsideGallery ensures clicks anywhere else on the page are ignored.
    this._clickHandler = (e: MouseEvent) => {
      if (!this._downInsideGallery) return;
    ...
    // mousedown scoped to container — prevents clicks elsewhere triggering onSelect
...
export interface CircularGalleryProps {
  ...
  scrollSpeed?: number;
  scrollEase?: number;
  /** Called with the image URL of the card closest to centre when the user clicks */
  onSelect?: (image: string) => void;
}
